...
def solve(start, state, target_vertices, all_length, limit, first=True):
    ...
    target_vertices - индексы непосещённых вершин
    ...
    if first:
        ...
        # обновляем индексы непосещённых вершин
        target_vertices1 = set(target_vertices) - set(arr)
        # выбираем новую отправную точку
        start = arr[-1]
        # обновляем длинну пройденого пути
        all_length += leng
        if not target_vertices1: # если непосещенных вершин не осталось - идём в 0 вершину
